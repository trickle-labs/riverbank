...
| [Use document distillation](use-document-distillation.md) | Reduce token cost pre-fragmentation with six configurable distillation strategies |
| [Control extraction focus](use-extraction-focus.md) | Tune precision vs recall with `comprehensive`, `high_precision`, and `facts_only` modes |
| [Use batch extraction](use-batch-extraction.md) | Group fragments into a single LLM call to reduce API costs |
| [Evaluate with Wikidata](evaluate-with-wikidata.md) | Measure extraction quality against Wikidata ground truth; analyse recall gaps |
